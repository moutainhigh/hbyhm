...
<%@ page import="com.tt.tool.ImgTools" %>
...
<%
  ...
  String sFileDef = request.getParameter("img_DefaultImgSrc");
	...
	int nw = 0 ;
	int nh = 0 ;
	boolean bDefImg = false;
%>
<%if(ntotal<=1){
  sFile = ii >ssImgs.length-1?sFileDef:ssImgs[ii] ;
	if (ii >ssImgs.length-1){
		bDefImg = true;
	}
  if (Tools.myIsNull(sFile)){
     sFile=sFileDef;   
  }else{
			String xsFile = Config.FILEUP_SAVEPATH + sFile;
			xsFile = xsFile.replaceFirst(Config.FILEUP_URLPRE, "/");
			xsFile = Tools.formatFilePath(xsFile);
			nw = ImgTools.getImgWidth(xsFile);
			nh = ImgTools.getImgHeight(xsFile);
	}
	String sSmallImg =Config.USESTATICIMG?Tools.urlImgStatic(sFile,120,120):Tools.urlImg(sFile,120,120);
  ii++;
  %>
<div style="position: relative;${param.l1div_Style}" id="div_${param.img_NamePre}${i}" class="gallerys">
  　<c:if test="${param.img_MarginImgSrc != null &&  param.img_MarginImgSrc !=''}">
    ...
    <img id="${param.img_NamePre}_view" name="${param.img_NamePre}_view" src="<%=sSmallImg%>" gal-src="/<%=sFile%>" class="${param.img_Class} gallery-pic" style="${param.img_Style}" gal-width="<%=nw%>" gal-height="<%=nh%>">
    ...
    <%--这个用来展示上传后的缩略图用--%>
    <input type="hidden" id="${param.img_NamePre}" name="${param.img_NamePre}" value="<%=bDefImg?"":sFile%>">
    <%--这个用来保存上传后的图片url值--%>
    <%--图片上传演示--%>
    <input type="file" id="upload_${param.img_NamePre}" runat="server" name="upload_immm" accept="image/*" style="${param.img_FileStyle}" class="${param.img_FileClass}">
		<div style="padding-top:10px;"><button type="button" class="btn btn-block btn-primary btn-xs" onclick="$.openPhotoGallery($('#${param.img_NamePre}_view${i}'));"><%=img_title%>
		</button></div>	
    <script>
      $('#upload_${param.img_NamePre}').fileUpload({//压缩图片为jpg后再上传
        "url": "/ttAjaxPost?do=fileup&smallwidth=${param.img_SmallWidth}&smallheight=${param.img_SmallHeight}",//smallwidth缩略图宽,smallheight缩略图高，shuitext水印文字
...
<div style="position: relative;${param.l1div_Style}" id="div_${param.img_NamePre}${i}">
  　<c:if test="${param.img_MarginImgSrc != null &&  param.img_MarginImgSrc !=''}">
    ...
    <img id="${param.img_NamePre}_view${i}" name="${param.img_NamePre}_view${i}" src="<%=sSmallImg%>" gal-src="/<%=sFile%>" class="${param.img_Class} gallery-pic" style="${param.img_Style}" gal-width="<%=nw%>" gal-height="<%=nh%>">
    <img id="${param.img_NamePre}_view${i}s" name="${param.img_NamePre}_view${i}s" style="float:center;width:12px;height:12px;text-align:center;display:none;" src="iframe/dist/img/loading/loading-spinner-grey.gif">
    <%--这个用来展示上传后的缩略图用--%>
