...
<%
    ...
    boolean canDel = request.getAttribute("canDel") == null ? false : (boolean) request.getAttribute("canDel");
    boolean canAdd = request.getAttribute("canAdd") == null ? true : (boolean) request.getAttribute("canAdd");
    ...
    <div class="admin-tools">
        <div class="row">
            <div class="col-sm-9 admin-button"><%-- col-sm-9，后面的数字9表示宽度，总共是12， --%>
                <div class="btn-group">
                    <c:if test="${canAdd}"><a href="<%=Tools.urlKill("id|sdo")+"&sdo=form"%>" data-target="#modal"
                                              class="btn btn-default"><i class="fa fa-edit"></i> 新增 </a>
                        <a data-toggle="modal" href="<%=Tools.urlKill("id|sdo")+"&sdo=float"%>" data-target="#modal"
                           class="btn btn-default"><i class="fa fa-edit"></i> 新增(modal模式)</a></c:if>
                    <c:if test="${canDel}"><a href="javascript:;" id="del_more_btn" class="btn btn-default"><i
                            class="fa fa-trash-o"></i> 删除</a></c:if>
                    <%if (havebtnfile) {%>
            ...
            <c:if test="${recs>0}">
                <div class="col-sm-3 admin-page-top hidden-xs">
                    <select id="page_limit_select" onchange="window.location.href='<%=sPlUrl%>&l='+this.value"
                            class="form-control">
                    ...
                    <div class="btn-group">
                        <a href="<%=sPlUrl%>&p=${p-1<=0?1:p-1}" class="btn btn-default">«</a> <a
                            href="<%=sPlUrl%>&p=${p+1>pages?pages:p+1}"
