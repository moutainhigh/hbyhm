...
$(document).ready(function () {
    $.post("/ttAjax?do=list&cn=comm_citys",function (data) {
       // alert("222222222");
        ...
        $('#search_cityid').selectpicker('val', '');
        $('#search_cityid').selectpicker('refresh');
        }
    )
    $.post("/ttAjax?do=list&cn=assess_fs",function (data) {
            // alert("222222222");
            var obj = eval('(' + data + ')');
            $("#saerch_fsid").empty();
            for(var i=0;i<obj.length;i++){
                $("#saerch_fsid").append("<option value='"+obj[i].id+"'>"+obj[i].name+"</option>");
            }
            $('#saerch_fsid').selectpicker('val', '');
            $('#saerch_fsid').selectpicker('refresh');
        }
    )
})
</script>
