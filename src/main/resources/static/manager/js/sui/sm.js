...
+function($) {
     ...
     */
    Router.prototype.load = function(url, ignoreCache, useAnimation) {
        ...
        if (this._isTheSameDocument(location.href, url)) {
	    	...
	    	this._saveDocumentIntoCache($(document), location.href, useAnimation);
        	if(!useAnimation){
        		this._switchToDocument(url, ignoreCache, true, DIRECTION.nothing);
        	}else{
    ...
    Router.prototype._doSwitchDocument = function(url, isPushState, direction) {
        ...
        if(this.cacheTitle[urlObj.base]){
        	this.nowPageTitle=this.cacheTitle[urlObj.base];
        }
     ...
     * @private
     */
    Router.prototype._saveDocumentIntoCache = function(doc, url, useAnimation) {
        if (useAnimation === undefined) {
        	useAnimation = true;
        }
         ...
         * 保存页面 title
         */
        if(!this.cacheTitle[urlAsKey]){
        	if($doc[0].title){
        		this.cacheTitle[urlAsKey]=$doc[0].title;
        	}else if($doc[0].outerHTML){
        		var re = new RegExp("<title>(.*)<\/title>");
        		var arr = re.exec($doc[0].outerHTML);
        		if(arr[1]){
        			this.cacheTitle[urlAsKey]=arr[1];
        		}
        	}
        }
        this.cache[urlAsKey] = {
    ...
    Router.prototype._animateElement = function($from, $to, direction) {
        ...
        var animPageClasses = [
            'page-from-show-to-hide',
            'page-from-hide-to-show',
        ...
        switch(direction) {
        	case DIRECTION.nothing:
                classForFrom = 'page-from-show-to-hide';
                classForTo = 'page-from-hide-to-show';
        		break;
    ...
    Router.prototype._back = function(state, fromState) {
        if (this._isTheSameDocument(state.url.full, fromState.url.full)) {
        	...
        	var urlAsKey = Util.toUrlObject(state.url.full).base;
        	if(this.cache[urlAsKey] && 'useAnimation' in this.cache[urlAsKey]){
        		var useAnimation = this.cache[urlAsKey].useAnimation;
        	}else{
        		var useAnimation = true;
        	}
            this._saveDocumentIntoCache($(document), fromState.url.full, useAnimation);
            if(useAnimation){
            	this._switchToDocument(state.url.full, false, false, DIRECTION.leftToRight);
            }else{
            	this._switchToDocument(state.url.full, false, false, DIRECTION.nothing);
            }
            this._saveAsCurrentState(state);
        }
    };
    ...
    $(function() {
        ...
        $(document).on('click', 'a', function(e) {
             ...
             * ray: 
             * 按照指定的顺序作页面左右切换判断
             * 顺序规则: 点击按钮元素的data-router-index属性(数字)
             */
            var routerIndex = $target.attr('data-router-index');
            //if (console) {
                //console.log(routerIndex,router.routerIndex);
            //}
            if (typeof routerIndex === 'undefined') {
                router.routerIndex = 0;
                router.directionDefault = DIRECTION.rightToLeft;
            } else if (routerIndex == '-1') {
                router.directionDefault = DIRECTION.leftToRight;
            } else if (!routerIndex || routerIndex<1) {
                router.directionDefault = DIRECTION.rightToLeft;
            } else {
                if (typeof router.routerIndex === 'undefined') {
                    router.routerIndex = 0;
                }
                if (router.routerIndex > routerIndex) {
                    router.directionDefault = DIRECTION.leftToRight;
                } else {
                    router.directionDefault = DIRECTION.rightToLeft;
                }
            ...
            if ($target.hasClass('back')) {
                ...
                var useAnimation = true;
                if(($target.attr('data-dont-animation') === 'true') || $target.hasClass('dont-animation')){
                	useAnimation=false;
                }

                router.load(url, ignoreCache, useAnimation);
            }
        });
    });
}(Zepto);
...
+function ($) {
  "use strict";
  $.lastPosition =function(options,id,arg) {
    if ( !sessionStorage) {
        return;
    }
    // 需要记忆模块的className
    ...
    needMemoryClass.forEach(function(item, index) {
        arg.find(item).on('click',function(){
        	//updateMemory(id,arg);
        });
    });
    getMemory(id,arg);
...
+function($) {
    ...
    $(window).on('pageInit', function(event,id,arg) {
        $.hideIndicator();
        $.lastPosition({
            needMemoryClass: [
                '.content'
