...
public class hxloan_dczy extends DbCtrl {

    private final String title = "电催作业";
    private String orderString = "ORDER BY dt_edit DESC"; // 默认排序
    private boolean canDel = false;
    private boolean canAdd = false;
    private final String classAgpId = "100"; // 随便填的，正式使用时应该跟model里此模块的ID相对应
    ...
    public void doGetList(HttpServletRequest request, TtMap post) {
        ...
        String fieldsString = "t.*, c.gems_code, c.c_name, c.c_cardno, c.loan_tpid, k.c_loaninfo_dkze dk_price, f.`name` fs_name, g.`name` gems_name"; // 显示字段列表如t.id,t.name,t.dt_edit,字段数显示越少加载速度越快，为空显示所有
        TtList list = null;
        /* 开始处理搜索过来的字段 */
        kw = post.get("kw");
        dtbe = post.get("dtbe");
