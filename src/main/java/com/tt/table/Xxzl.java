 ...
 * @LastEditTime: 2019-06-20 10:12:34
 * @LastEditors: tt
 */
...
public class Xxzl extends DbCtrl {
	...
	public void succ(TtMap array, long id, int sqltp) {
		...
		Tools.recEdit(map, "hbyh_xxzl", id);
		// 历史添加
		TtMap res = new TtMap();
		...
		if (StringUtils.isNotEmpty(array.get("mid_add")) && array.get("mid_add").equals(array.get("mid_edit"))) {
			Addadmin_msg.addmsg(array.get("mid_edit"), array.get("bc_status"), array.get("remark1"), recinfo.get("c_name"),
					"贷款材料", "河北银行", array.get("mid_add"));
		} else {
			Addadmin_msg.addmsg(array.get("mid_add"), array.get("bc_status"), array.get("remark1"), recinfo.get("c_name"),
					"贷款材料", "河北银行", array.get("mid_add"));
			Addadmin_msg.addmsg(array.get("mid_edit"), array.get("bc_status"), array.get("remark1"), recinfo.get("c_name"),
					"贷款材料", "河北银行", array.get("mid_add"));
		}
	...
	public void doGetList(HttpServletRequest request, TtMap post) {
		...
		String tmpWhere = "";
		String fieldsString = "t.*" + ",f.name as fsname" + ",f.id as fsid" + ",cs.name as state_name"+",i.checkname as checkname"
				+ ",cc.name as city_name" + ",a.name as adminname" + ",i.c_name as c_name" + ",aa.name as aa_name";
		...
		leftsql = "LEFT JOIN assess_fs f ON f.id=t.gems_fs_id " + "LEFT JOIN assess_gems a ON a.id=t.gems_id "
				+ "LEFT JOIN kj_icbc i ON i.id=t.icbc_id " + "LEFT JOIN assess_admin aa ON aa.id=i.current_editor_id"
	...
	public boolean chk(TtMap array, long id) {
		...
		if (!Tools.myIsNull(array.get("fromcommand"))) { // 从ManagerCmd来的。不用过滤参数
			...
			// myErroMsg += "出厂日期不能为空！\n";
			// }
			// if(Tools.myIsNull(array.get("carkm"))){
			// myErroMsg += "公里数不能为空！\n";
			// }
