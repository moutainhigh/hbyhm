 ...
 * @Author: tt
 * @Date: 2019-06-17 14:41:33
 * @LastEditTime: 2019-06-18 15:33:21
 * @LastEditors: tt
 */
...
public class xmgj_dygd extends DbCtrl {
    private final String title = "抵押归档";
    private String orderString = "ORDER BY dt_edit DESC"; // 默认排序
    private boolean canDel = true;
    private boolean canAdd = true;
    private final String classAgpId = "73"; // 随便填的，正式使用时应该跟model里此模块的ID相对应
    ...
    public void doGetForm(HttpServletRequest request, TtMap post) {
        ...
        if(!Tools.myIsNull(post.get("toZip"))&& post.get("toZip").equals("1")) {
            ...
            //历史操作查询
            if (nid > 0) {
                TtList lslist = Tools.reclist("select * from xmgj_dygd_result where qryid=" + nid);
            ...
            Tools.recEdit(map, "xmgj_dygd", nid);
    ...
    public void doPost(TtMap post, long id, TtMap result2) {
        ...
        Tools.recAdd(res, "xmgj_dygd_result");
        ...
        if(StringUtils.isNotEmpty(newpost.get("mid_add")) && newpost.get("mid_add").equals(newpost.get("mid_edit"))){
            Addadmin_msg.addmsg(newpost.get("mid_edit"), newpost.get("bc_status"), newpost.get("remark1"), recinfo.get("c_name"), "抵押归档", "厦门国际银行", newpost.get("mid_add"));

        } else {
            Addadmin_msg.addmsg(newpost.get("mid_add"), newpost.get("bc_status"), newpost.get("remark1"), recinfo.get("c_name"),"抵押归档","厦门国际银行", newpost.get("mid_add"));
            Addadmin_msg.addmsg(newpost.get("mid_edit"), newpost.get("bc_status"), newpost.get("remark1"), recinfo.get("c_name"), "抵押归档", "厦门国际银行", newpost.get("mid_add"));
    ...
    public void doGetList(HttpServletRequest request, TtMap post) {
        ...
        String fieldsString = "t.*,f.name as fsname" +
                ...
                ",a.name as adminname,i.c_name as c_name,i.fk_status,i.id as icbc_id,aa.name as aa_name"+",i.checkname as checkname";
