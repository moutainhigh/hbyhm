...
public class hxyh_gsclhs extends DbCtrl {
    private final String title = "全部材料回收";
    private String orderString = "ORDER BY dt_edit DESC"; // 默认排序
    private boolean canDel = true;
    private boolean canAdd = true;
    private final String classAgpId = "65"; // 随便填的，正式使用时应该跟model里此模块的ID相对应
    ...
    public void doGetForm(HttpServletRequest request, TtMap post) {
        ...
        //历史操作查询
        if (nid > 0) {
            TtList lslist = Tools.reclist("select * from hxyh_gsclhs_result where qryid=" + nid);
        ...
        Tools.recEdit(map, "hxyh_gsclhs", nid);
    ...
    public void doPost(TtMap post, long id, TtMap result2) {
        ...
        Tools.recAdd(res, "hxyh_gsclhs_result");
        ...
        if(StringUtils.isNotEmpty(newpost.get("mid_add")) && newpost.get("mid_add").equals(newpost.get("mid_edit"))){
            Addadmin_msg.addmsg(newpost.get("mid_edit"), newpost.get("bc_status"), newpost.get("remark1"), recinfo.get("c_name"), "全部材料", "华夏银行", newpost.get("mid_add"));

        } else {
            Addadmin_msg.addmsg(newpost.get("mid_add"), newpost.get("bc_status"), newpost.get("remark1"), recinfo.get("c_name"),"全部材料","华夏银行", newpost.get("mid_add"));
            Addadmin_msg.addmsg(newpost.get("mid_edit"), newpost.get("bc_status"), newpost.get("remark1"), recinfo.get("c_name"), "全部材料", "华夏银行", newpost.get("mid_add"));
