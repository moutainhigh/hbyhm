...
public class hxyh_gpsgd extends DbCtrl {
    ...
    private final String classAgpId = "62"; // 随便填的，正式使用时应该跟model里此模块的ID相对应
    ...
    public void doGetForm(HttpServletRequest request, TtMap post) {
        String f = "t.*,a.name as admin_name,fs.name as fs_name,i.c_name as c_name,xx.vincode as vincode,i.id as icbc_id";
...
/*            TtMap imgstep14_1ss=tozip(info.get("imgstep14_1ss"),"车身照片车身照片（定位水印）");
            ...
            Tools.recEdit(map, "hxyh_gpsgd", nid);
    ...
    public void doPost(TtMap post, long id, TtMap result2) {
        ...
        if(StringUtils.isNotEmpty(newpost.get("mid_add")) && newpost.get("mid_add").equals(newpost.get("mid_edit"))){
            Addadmin_msg.addmsg(newpost.get("mid_edit"), newpost.get("bc_status"), newpost.get("remark1"), recinfo.get("c_name"), "GPS归档", "华夏银行", newpost.get("mid_add"));

        } else {
            Addadmin_msg.addmsg(newpost.get("mid_add"), newpost.get("bc_status"), newpost.get("remark1"), recinfo.get("c_name"),"GPS归档","华夏银行", newpost.get("mid_add"));
            Addadmin_msg.addmsg(newpost.get("mid_edit"), newpost.get("bc_status"), newpost.get("remark1"), recinfo.get("c_name"), "GPS归档", "华夏银行", newpost.get("mid_add"));
