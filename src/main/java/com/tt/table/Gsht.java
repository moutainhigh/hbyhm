...
public class Gsht extends DbCtrl {
    ...
    public void doPost(TtMap post, long id, TtMap result2) {
        ...
        if(StringUtils.isNotEmpty(post.get("mid_add")) && post.get("mid_add").equals(post.get("mid_edit"))){
            Addadmin_msg.addmsg(post.get("mid_edit"), post.get("bc_status"), newpost.get("remark1"), post.get("c_name"), "投保材料", "河北银行", post.get("mid_add"));

        } else {
            Addadmin_msg.addmsg(post.get("mid_add"), post.get("bc_status"), newpost.get("remark1"), post.get("c_name"),"投保材料","河北银行", post.get("mid_add"));
            Addadmin_msg.addmsg(post.get("mid_edit"), post.get("bc_status"), newpost.get("remark1"), post.get("c_name"), "投保材料", "河北银行", post.get("mid_add"));
