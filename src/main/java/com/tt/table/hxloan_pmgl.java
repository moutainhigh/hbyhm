...
public class hxloan_pmgl extends DbCtrl {

    private final String title = "拍卖管理";
    private String orderString = "ORDER BY dt_edit DESC"; // 默认排序
    private boolean canDel = false;
    private boolean canAdd = false;
    private final String classAgpId = "102"; // 随便填的，正式使用时应该跟model里此模块的ID相对应
    public boolean agpOK = false;// 默认无权限

    public hxloan_pmgl(){
        super("hxloan_overdue_list");
    ...
    public void doGetList(HttpServletRequest request, TtMap post) {
        ...
        String tmpWhere = " and t.type_id = 5";
        System.out.println("............."+post.get("tctype"));
        if ("1".equals(post.get("pmtype"))){     //未拍卖
            tmpWhere = " and t.type_id = 5 and t.type_status = 51";
        }
        if ("2".equals(post.get("pmtype"))){     //亏损(拍卖完成)
            tmpWhere = " and t.type_id = 5 and t.type_status = 52";
        }
        if ("3".equals(post.get("pmtype"))){     //盈利(拍卖完成)
            tmpWhere = " and t.type_id = 5 and t.type_status = 53";
        }
        ...
        leftsql= "LEFT JOIN kj_icbc c on c.id=t.icbc_id " +
                "LEFT JOIN hxyh_xxzl k on k.icbc_id=t.icbc_id " +
    ...
    public void doGetForm(HttpServletRequest request, TtMap post) {
        ...
        String bbsql = "select * from hxloan_overdue_list where id = " + nid;
        ...
        String sql = "SELECT SQL_CALC_FOUND_ROWS\n" +
                ...
                "\tFROM\n" +
                "\tkj_icbc t\n" +
                "\tLEFT JOIN hxyh_xxzl k ON k.icbc_id = t.id\n" +
        ...
        //还款计划
        String hkjhsql = "SELECT * FROM hxloan_repayment_schedule WHERE icbc_id = " + bbmap.get("icbc_id");
        TtList reclist = Tools.reclist(hkjhsql);

        //贷后信息
        String dhsql = "select k.*,a.`name` gems_name,f.`name` fs_name from hxyh_xxzl k left join assess_gems a on a.id=k.gems_id left join assess_fs f on f.id=k.gems_fs_id where k.icbc_id = " + bbmap.get("icbc_id");
        TtMap mapafter = Tools.recinfo(dhsql);

        //记录栏
        String jlsql = "select lo.*,a.`name` gems_name from hxloan_overdue_list_result lo left join assess_gems a on a.id = lo.mid_add where lo.icbc_id = " + bbmap.get("icbc_id");
        TtList jllist = Tools.reclist(jlsql);

        //电催的配置
        String configSql = "select * from hxloan_config where gems_fs_id="+Tools.minfo().get("icbc_erp_fsid");
        ...
        request.setAttribute("pmtype", post.get("pmtype"));
