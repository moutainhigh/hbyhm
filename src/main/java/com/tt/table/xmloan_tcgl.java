...
public class xmloan_tcgl extends DbCtrl {

    private final String title = "拖车管理";
    private String orderString = "ORDER BY dt_edit DESC"; // 默认排序
    private boolean canDel = false;
    private boolean canAdd = false;
    private final String classAgpId = "92"; // 随便填的，正式使用时应该跟model里此模块的ID相对应
...
//        Stirng consql = "select * from loan_config";
    ...
    public void doGetForm(HttpServletRequest request, TtMap post) {
        ...
        if(bbmap.get("type_status").equals("33") || bbmap.get("type_status").equals("32") ){
            String mapssql = "select k.*,k.result_value from xmloan_overdue_list_result k where k.type_status=33 and k.icbc_id ="+bbmap.get("icbc_id")+" ORDER BY dt_add DESC limit 0,1";
