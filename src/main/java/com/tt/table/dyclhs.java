...
public class dyclhs extends DbCtrl {
	...
	public void doGetForm(HttpServletRequest request, TtMap post) {
		...
		Tools.recEdit(map, "hbyh_dyclhs", nid);
	...
	public void doPost(TtMap post, long id, TtMap result2) {
		...
		if (StringUtils.isNotEmpty(newpost.get("mid_add")) && newpost.get("mid_add").equals(newpost.get("mid_edit"))) {
			Addadmin_msg.addmsg(newpost.get("mid_edit"), newpost.get("bc_status"), newpost.get("remark1"),
					recinfo.get("c_name"), "补寄材料", "河北银行", newpost.get("mid_add"));
		} else {
			Addadmin_msg.addmsg(newpost.get("mid_add"), newpost.get("bc_status"), newpost.get("remark1"),
					recinfo.get("c_name"), "补寄材料", "河北银行", newpost.get("mid_add"));
			Addadmin_msg.addmsg(newpost.get("mid_edit"), newpost.get("bc_status"), newpost.get("remark1"),
					recinfo.get("c_name"), "补寄材料", "河北银行", newpost.get("mid_add"));
		}
	...
	public void doGetList(HttpServletRequest request, TtMap post) {
		...
		leftsql = "LEFT JOIN assess_fs f ON f.id=t.gems_fs_id " + "LEFT JOIN assess_gems a ON a.id=t.gems_id "
				+ "LEFT JOIN kj_icbc i ON i.id=t.icbc_id " + " LEFT JOIN assess_admin admin ON admin.gemsid=a.id "
				+ " LEFT JOIN comm_states cs ON cs.id=admin.stateid " + " LEFT JOIN comm_citys cc ON cc.id=admin.cityid "
