...
public class hxyh_xxzl extends DbCtrl {
    ...
    private final String classAgpId = "59"; // 随便填的，正式使用时应该跟model里此模块的ID相对应
    ...
    public void doGetForm(HttpServletRequest request, TtMap post) {
        ...
        if (!Tools.myIsNull(post.get("toZip")) && post.get("toZip").equals("1")) {
            ...
            //历史操作查询
            if (nid > 0) {
                TtList lslist = Tools.reclist("select * from hxyh_xxzl_result where qryid=" + nid);
    ...
    public void doPost(TtMap post, long id, TtMap result2) {
        ...
        if(StringUtils.isNotEmpty(post.get("mid_add")) && post.get("mid_add").equals(post.get("mid_edit"))){
            Addadmin_msg.addmsg(post.get("mid_edit"), post.get("bc_status"), newpost.get("remark1"), post.get("c_name"), "贷款材料", "华夏银行", post.get("mid_add"));

        } else {
            Addadmin_msg.addmsg(post.get("mid_add"), post.get("bc_status"), newpost.get("remark1"), post.get("c_name"),"贷款材料","华夏银行", post.get("mid_add"));
            Addadmin_msg.addmsg(post.get("mid_edit"), post.get("bc_status"), newpost.get("remark1"), post.get("c_name"), "贷款材料", "华夏银行", post.get("mid_add"));
