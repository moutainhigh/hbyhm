...
public class hxloan_khhkqk extends DbCtrl {

    private final String title = "客户还款情况";
    private String orderString = "GROUP BY icbc_id ORDER BY t.dt_edit DESC"; // 默认排序
    private boolean canDel = false;
    private boolean canAdd = false;
    private final String classAgpId = "98"; // 随便填的，正式使用时应该跟model里此模块的ID相对应
    public boolean agpOK = false;// 默认无权限

    public hxloan_khhkqk() {
    ...
    public void doGetList(HttpServletRequest request, TtMap post) {
        ...
        String whereString = "t.qd_type=4";   // 2=河北银行   3=厦门银行   4=华夏银行
        String tmpWhere = "";
        ...
        leftsql="LEFT JOIN hxyh_xxzl xx on xx.icbc_id=t.icbc_id " +
    ...
    public void doGetForm(HttpServletRequest request, TtMap post) {
        ...
        String sql = "select SQL_CALC_FOUND_ROWS t.*,xx.*,xx.c_loaninfo_dkze,xx.c_loaninfo_periods,xx.c_loaninfo_car_priceresult from kj_icbc t LEFT JOIN hxyh_xxzl xx on xx.icbc_id=t.id where t.id = " + nid;
        ...
        String hkjhsql = "SELECT * FROM hxloan_repayment_schedule WHERE icbc_id = " + nid;
        TtList reclist = Tools.reclist(hkjhsql);

        //贷后信息
        String dhsql = "select * from hxyh_xxzl where icbc_id=" + nid;
