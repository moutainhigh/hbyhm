...
public class hxyh_yhht extends DbCtrl {
    private final String title = "签约材料";
    private String orderString = "ORDER BY dt_edit DESC"; // 默认排序
    private boolean canDel = true;
    private boolean canAdd = true;
    private final String classAgpId = "60"; // 随便填的，正式使用时应该跟model里此模块的ID相对应
    ...
    public void doGetForm(HttpServletRequest request, TtMap post) {
        ...
        if (!Tools.myIsNull(post.get("toZip")) && post.get("toZip").equals("1")) {
            ...
            Tools.recEdit(map, "hbyh_yhht", nid);
    ...
    public void imgs(TtMap post) {
        String imgstep12_1ss = "";
        if (!post.get("imgstep12_1ss_num").isEmpty() && !post.get("imgstep12_1ss_num").equals("")) {
            int imgstep12_1ss_num = Integer.parseInt(post.get("imgstep12_1ss_num"));
            for (int i = 1; i <= imgstep12_1ss_num; i++) {
                imgstep12_1ss = imgstep12_1ss + post.get("imgstep12_1ss" + i) + "\u0005";
            }
        }
        post.put("imgstep12_1ss", imgstep12_1ss);
    }
    ...
    public void doPost(TtMap post, long id, TtMap result2) {
        ...
        Tools.recAdd(res, "hxyh_yhht_result");
        ...
        if(StringUtils.isNotEmpty(newpost.get("mid_add")) && newpost.get("mid_add").equals(newpost.get("mid_edit"))){
            Addadmin_msg.addmsg(newpost.get("mid_edit"), newpost.get("bc_status"), newpost.get("remark1"), recinfo.get("c_name"), "签约材料", "华夏银行", newpost.get("mid_add"));

        } else {
            Addadmin_msg.addmsg(newpost.get("mid_add"), newpost.get("bc_status"), newpost.get("remark1"), recinfo.get("c_name"),"签约材料","华夏银行", newpost.get("mid_add"));
            Addadmin_msg.addmsg(newpost.get("mid_edit"), newpost.get("bc_status"), newpost.get("remark1"), recinfo.get("c_name"), "签约材料", "华夏银行", newpost.get("mid_add"));
    ...
    public void doGetList(HttpServletRequest request, TtMap post) {
        ...
        String fieldsString = "t.*" +
                ...
                ",f.name as fsname,a.name as adminname,i.c_name as c_name,aa.name as aa_name"+",i.checkname as checkname";
