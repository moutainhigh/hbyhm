...
public class xmgj_gpsgd extends DbCtrl {
    private final String title = "GPS归档";
    private String orderString = "ORDER BY dt_edit DESC"; // 默认排序
    private boolean canDel = true;
    private boolean canAdd = true;
    private final String classAgpId = "72"; // 随便填的，正式使用时应该跟model里此模块的ID相对应
    ...
    public void doGetForm(HttpServletRequest request, TtMap post) {
        ...
        leftsql = " LEFT JOIN assess_gems a ON a.id=t.gems_id" +
                ...
                " LEFT JOIN xmgj_xxzl xx ON xx.icbc_id=t.icbc_id" +
        ...
        if(!Tools.myIsNull(post.get("toZip"))&& post.get("toZip").equals("1")) {
            ...
            //历史操作查询
            if (nid > 0) {
                TtList lslist = Tools.reclist("select * from xmgj_gpsgd_result where qryid=" + nid);
            ...
            Tools.recEdit(map, "xmgj_gpsgd", nid);
