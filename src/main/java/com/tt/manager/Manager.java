...
public class Manager {
	...
	public String index(String cn, String type, String sdo, String id, HttpServletRequest request,
		...
		TtMap post = Tools.getPostMap(request, false);// 过滤参数，过滤mysql的注入，url参数注入
		if (!Tools.myIsNull(post.get("ttDebugMode"))) { // DEBUGMODE通过命令行传入
			Config.setDebugMode(post.get("ttDebugMode").equals("1"));
		}
			...
			case "login":/** 登陆 */
				...
				if (info.size() > 0) {
					Tools.formatResult(result2, true, 0, "登陆成功！",
							Tools.myIsNull(refer) ? "/manager/index?cn=homeHx&sdo=form&type=demo"
									: URLDecoder.decode(refer, "UTF-8"));
