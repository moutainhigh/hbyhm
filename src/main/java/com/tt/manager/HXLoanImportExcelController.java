 ...
 */
@Controller
public class HXLoanImportExcelController {
    ...
    @RequestMapping(value="/manager/importExcelControllerhx",method= RequestMethod.POST)
    @ResponseBody
...
//							c = recordService.count(cells[cellNum]);
                    ...
                    long loan_import_excels = Tools.recAdd(rowMap, "hxloan_import_excels");//把excel表格中的数据录入数据库表loan_import_excels
                    ...
                    if (overdue_amount > 0) { //该人逾期金额大于0
                        ...
                        String yqsql = "select * from hxloan_overdue_list where c_cardno = " + rowMap.get("id_card");
                        TtMap yqmap = new TtMap();
                        yqmap = Tools.recinfo(yqsql);
                        if (0 == yqmap.size()) {
                            System.out.println("逾期表里没有此人!");
                            String sql = "select k.*,hxxx.carno as c_carno,hxxx.vincode as c_carvin from kj_icbc k,hxyh_xxzl hxxx where c_cardno="+rowMap.get("id_card")+"and hxxx.icbc_id=k.id";
...
//						addOverdueClient.put("type_status",LoanModel.LoanTypeModel().get("初级逾期") );//???
                            ...
                            long loan_overdue_list = Tools.recAdd(addOverdueClient, "hxloan_overdue_list");
...
//                        upPay.put("icbc_id",(recinfo==null?null:recinfo.get("icbc_id")));
                        //修改还款计划表
                        String hksql = "update hxloan_repayment_schedule set\n" +
                                "dt_edit='" + Getnow() + "', \n" +
...
//        System.out.println("mmmmmmm   " + minfo);
        ...
        map.put("fsname", "");// 公司名字
        long loan_import_record = Tools.recAdd(map, "hxloan_import_record");//把excel表格中的数据录入数据库表loan_import_excels //添加excel导入文件记录
